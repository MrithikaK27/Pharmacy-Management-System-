...
class MYPDF extends TCPDF {
	...
	public function ColoredTable($header,$data) {
		...
		foreach($data as $row) {
			...
			$this->Cell($w[4], 4, number_format((float)$row[4],2), 'LR', 0, 'R', $fill);
			$this->Cell($w[5], 4, number_format((float)$row[5],2), 'LR', 0, 'R', $fill);
			$this->Ln();
			$fill=!$fill;
		}
...
Student Center Ground Floor,<br> P.O. Box Private Bag Maseno, Kenya <br> Tel: +254 071 214 356 <br> E-mail: pharmacyManagement System	@yahoo.com 
<br>-----------------------------------------
EOD;
